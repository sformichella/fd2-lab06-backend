...
app.post('/theorems', async(req, res) => {
  try {
    ...
    const newFieldId = req.body.field_id;
    ...
    const data = await client.query(`
      ...
      RETURNING *
    `, 
    [newName, newDifficulty, newVeracity, newFieldId, newOwnerId]
    );
...
app.put('/theorems/:id', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      WHERE theorems.id = $6
      RETURNING *
      `, [newName, newDifficulty, newVeracity, newFieldId, newOwnerId, req.params.id]
    );
