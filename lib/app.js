...
app.get('/theorems', async(req, res) => {
  try {
    const data = await client.query('SELECT * from theorems');
...
app.post('/theorems', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      INSERT INTO theorems (name, difficulty, veracity, field)
      VALUES ($1, $2, $3, $4)
      RETURNING *
    `, 
    [newName, newDifficulty, newVeracity, newField]
    );
...
app.put('theorems/:id', async(req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      WHERE theorems.id = $5
      RETURNING *
    `, [newName, newDifficulty, newVeracity, newField, req.params.id]
    );
